...
module.exports = {
  ...
  */
  ...
  // Important: This means pets db must be specified in config.js, else an error will occur when query is run
    ...
    user: {
        profile: 'select name, email, reward from qzsp4l888kg17t5l.user where user_id=?',
        checkEmailExists: 'select * from user where email=?',
        checkUserNameExists: 'select user_id from qzsp4l888kg17t5l.user where name=?',
        ...
        getAvailableRewardsIgnoringTransaction: 'SELECT sum(R.change) as sum FROM qzsp4l888kg17t5l.reward R where user_id=? and date_active <= curdate() and (transaction_id != ? or transaction_id is NULL);',
        getBookingForTransaction:'SELECT * FROM qzsp4l888kg17t5l.booking WHERE transaction_id=?',
  ...
  hotel: {
    ...
    search: function (params = {}, getCount = false) {
      ...
      */
      ...
      from qzsp4l888kg17t5l.booking B join qzsp4l888kg17t5l.room R 
  ...
  booking: {
     ...
     */
    makeTransaction: 'INSERT INTO qzsp4l888kg17t5l.transaction(transaction_id, user_id, guest_id, total_price, cancellation_charge, date_in, date_out, status, amount_paid, stripe_id) values (null, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
     ...
     * @returns A formatted query ie "INSERT INTO qzsp4l888kg17t5l.transaction_room(transaction_id, room_id, room_price) VALUES (39,10,20),(39,11,65)"
     */
    makeTransactionDetails: function (transaction_id, rooms_booked) {
      let insertStatement = "INSERT INTO qzsp4l888kg17t5l.transaction_room(transaction_id, room_id, room_price) VALUES "
      let placeholders = []
      let values = []
    ...
    cancel_transaction: 'UPDATE qzsp4l888kg17t5l.transaction SET status="cancelled" WHERE transaction_id=?',
    cancel_all: 'DELETE from qzsp4l888kg17t5l.transaction_room where transaction_id=?',
    // For when the user cancels only a single room
    cancel_one: 'DELETE from qzsp4l888kg17t5l.transaction_room where transaction_id=? AND room_id=?',
    cancel_one_room: 'UPDATE qzsp4l888kg17t5l.transaction SET total_price=?, cancellation_charge=?, amount_paid=? WHERE transaction_id=?',
    ...
    isCancellable: function ({ transaction_id }) {
      let query = `SELECT * FROM qzsp4l888kg17t5l.transaction WHERE
    ...
    duplicateBookingCheck: function ({ user_id, date_in, date_out }) {
      let query = `
        SELECT 
          ...
          R.bed_type
        FROM
            qzsp4l888kg17t5l.booking B
                JOIN
            qzsp4l888kg17t5l.room R ON B.room_id = R.room_id
        WHERE
            date_in < ?
                AND date_out > ?
                AND user_id = ?
   ...
   * 
    */
      ...
      FROM
          (SELECT 
              NOT EXISTS( SELECT 
                      ...
                      FROM
                          qzsp4l888kg17t5l.booking B
                      JOIN qzsp4l888kg17t5l.room R ON B.room_id = R.room_id
                      WHERE
                          date_in < ?
                              AND date_out > ?
                              AND status != 'cancelled'
    ...
    rewards: {
      book: 'INSERT INTO qzsp4l888kg17t5l.rewards(reward_book_id, user_id, room_id, reward_points, no_cancellation, date_in, date_out, status) values (null, ?, ?, ?, ?, ?, ?, ?)',
      book: 'INSERT INTO qzsp4l888kg17t5l.rewards (reward_book_id, user_id, room_id, reward_points, no_cancellation, date_in, date_out, status) values (null, ?, ?, ?, ?, ?, ?, ?)',
      useOnBooking: 'INSERT INTO qzsp4l888kg17t5l.reward (reward_id, user_id, reward_reason_id, transaction_id, date_active, `change`) values (null, ?, 1, ?, curdate(), ?)',
      gainFromBooking: 'INSERT INTO qzsp4l888kg17t5l.reward (reward_id, user_id, reward_reason_id, transaction_id, date_active, `change`) values (null, ?, 2, ?, ?, ?)',
      getUserRecords: 'SELECT R.*,RR.reason FROM qzsp4l888kg17t5l.reward R join qzsp4l888kg17t5l.reward_reason RR on R.reward_reason_id = RR.reward_reason_id WHERE user_id=?',
      cancelBooking: 'DELETE from qzsp4l888kg17t5l.reward where transaction_id=?',
      getOldBookingAppliedRewards: 'SELECT R.change FROM qzsp4l888kg17t5l.reward R WHERE transaction_id = ? AND SIGN(R.change) = -1',
      getAppliedRewards: 'SELECT R.change FROM qzsp4l888kg17t5l.reward R WHERE transaction_id = ? AND user_id = ? AND SIGN(R.change) = -1',
  ...
  guest: {
    insert: 'INSERT INTO qzsp4l888kg17t5l.guest(guest_id, email, name) values (null, ?, ?)'
  },
  modify: {
    removeTransactionRoomDataAndRewardsForTransaction: `
      ...
      TR.transaction_id = ?
      `,
    updateTransaction: 'UPDATE qzsp4l888kg17t5l.transaction SET total_price=?, cancellation_charge=?, date_in=?, date_out=?, status=?, amount_paid=?, stripe_id=? WHERE transaction_id=?',
    getExistingTransaction: `
      ...
      R.bed_type, group_concat( distinct(url) ) as images, count(R.room_id) as quantity, group_concat(R.room_id) as room_ids FROM qzsp4l888kg17t5l.booking B
	    join room R
  ...
  email:{
      getHotelInfo: (hotelID)=>{
        let q1 = 'select * from qzsp4l888kg17t5l.hotel where hotel_id = ?'
 ...
 */
  transaction: {
    getRoomInfo: `SELECT T.*, group_concat(TR.transaction_room_id) as transaction_room_ids, group_concat(TR.room_id) as room_ids, ANY_VALUE(TR.room_price) as room_price, ANY_VALUE(R.hotel_id) as hotel_id, group_concat(R.room_number), R.bed_type, ANY_VALUE(R.capacity) as capacity, ANY_VALUE(RI.url) as image,
                  COUNT(*) as quantity
                  FROM qzsp4l888kg17t5l.transaction T
                  join qzsp4l888kg17t5l.transaction_room TR
                  on T.transaction_id = TR.transaction_id
                  join qzsp4l888kg17t5l.room R
                  on TR.room_id = R.room_id
                  join qzsp4l888kg17t5l.room_image RI
